...
class FileTransfer extends PureComponent {
  ...
  constructor(props) {
    ...
    let { room } = props;
    room = room.replace(/-/g, ' ');
    const savedData = JSON.parse(localStorage.getItem('filedrop'));
    this.client = {
      ...savedData.user,
      room,
    };
  ...
  componentDidMount() {
    document.title = `${this.client.room} block | FileDrop`;
  ...
  handleShare = () => {
    ...
    navigator.share({
      title: 'Share files',
      text: `Join my block '${this.client.room}' on FileDrop to share files`,
  ...
  copyLink = () => {
    if (navigator.share)
      ...
      toast('Block link copied to clipboard');
    }
  }
      ...
      <div class="file-transfer">
        <header class="app-header">
          ...
          <button
            ...
            aria-label={navigator.share ? 'Share block link' : 'Copy block link'}
              ...
              peers.length <= 1 && (
                <div class="share-room-link">
                  ...
                  <button class="outlined" onClick={navigator.share ? this.handleShare :this.copyLink}>
                    {navigator.share ? 'Share block link' : 'Copy block link'}
                  </button>
                </div>
              )
            }
        ...
        { isSelectorEnabled && <FileDropper onFile={this.selectFiles} />}
      </div>
    );
  }
}
